...
test("release gate has fixed, bounded text and media canaries", () => {
  assert.equal(LIVE_WORKLOADS.length, 10);
  assert.deepEqual(
    ...
    [
      ["z-image-turbo", "image"],
      ["LTX Director 2.0", "video"],
      ["ace-step-v1.5-xl-turbo", "audio"],
    ],
  );
  assert.equal(
    LIVE_WORKLOADS.find(({ modality }) => modality === "video")?.seconds,
    1,
  );
  assert.equal(
    LIVE_WORKLOADS.find(({ modality }) => modality === "audio")?.seconds,
    10,
  );
...
test("quoted bounds match every child live test", async () => {
  const sources = await Promise.all(
    [
      ...
      "n8n-nodes-aipg/test/live.e2e.test.mjs",
    ].map((path) => readFile(new URL(`../${path}`, import.meta.url), "utf8")),
  );
  const [dify, aiSdk, eliza, langchain, n8n] = sources;
  ...
  const results = LIVE_WORKLOADS.map(({ id }, index) => ({
    ...
    payload: {
      ...
      estimate: {
        ...
        cost_micro: index < 7 ? 10 : [3_000, 20_000, 2_000][index - 7],
      },
    },
  }));
  assert.equal(validateQuoteResults(results, HARD_SPEND_CAP_MICRO), 25_070);
});
