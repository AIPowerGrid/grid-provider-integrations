...
`@aipowergrid/ai-sdk-provider@0.1.0` is public with npm provenance from tag
`ai-sdk-provider-v0.1.0`. Future releases publish only through the root
`publish-packages.yml` workflow and its npm OIDC Trusted Publisher. The
one-time bootstrap token is revoked; do not restore a registry token.
